...
The Ethereum community is in the process of migrating from the existing proof of work (PoW) consensus mechanism to proof of stake (PoS), which is more energy-friendly. This migration has required an elaborate process to spin up a separate, new type of blockchain called a Beacon chain that is being merged into the existing, main Ethereum blockchain. The Ethereum Foundation estimated this will reduce energy use by 99.95%.
